-- Criação da tabela modulos_curso se não existir
...
-- Criação da tabela licoes_modulo se não existir
...
-- Políticas de segurança para modulos_curso
CREATE POLICY "Admins podem gerenciar módulos"
ON public.modulos_curso
FOR ALL
TO authenticated
USING (
    EXISTS (
...
-- Políticas de segurança para licoes_modulo
CREATE POLICY "Admins podem gerenciar lições"
ON public.licoes_modulo
FOR ALL
TO authenticated
USING (
    EXISTS (
...
CREATE POLICY "Qualquer um pode ver lições ativas"
ON public.licoes_modulo
FOR SELECT
TO authenticated
USING (ativo = true);
...
-- Buscar o ID do curso recém criado para usar nas próximas inserções
DO $$
DECLARE
    curso_id_var UUID;
    modulo_id_var UUID;
BEGIN
    ...
    SELECT id INTO curso_id_var FROM public.cursos WHERE nome = 'Escola de Líderes DNA' LIMIT 1;
