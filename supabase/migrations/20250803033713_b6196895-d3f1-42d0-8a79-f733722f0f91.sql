-- Criar enum para papéis da igreja (somente se não existir)
DO $$ BEGIN
...
-- Criar tabela de papéis (se não existir)
CREATE TABLE IF NOT EXISTS papeis_igreja (
...
-- Criar tabela de permissões granulares (se não existir)
...
-- Atualizar tabela pessoas para incluir papel (se coluna não existir)
DO $$ BEGIN
...
-- Inserir dados iniciais dos papéis (com ON CONFLICT)
...
-- Inserir dados iniciais dos módulos (com ON CONFLICT)
...
-- Criar políticas RLS (com DROP IF EXISTS para evitar erros)
DROP POLICY IF EXISTS "Qualquer um pode ver papéis ativos" ON papeis_igreja;
CREATE POLICY "Qualquer um pode ver papéis ativos" ON papeis_igreja
  FOR SELECT USING (ativo = true);

DROP POLICY IF EXISTS "Qualquer um pode ver módulos ativos" ON modulos_sistema;
CREATE POLICY "Qualquer um pode ver módulos ativos" ON modulos_sistema
  FOR SELECT USING (ativo = true);

DROP POLICY IF EXISTS "Admins podem gerenciar permissões" ON permissoes_sistema;
...
DROP POLICY IF EXISTS "Usuários podem ver próprias permissões" ON permissoes_sistema;
