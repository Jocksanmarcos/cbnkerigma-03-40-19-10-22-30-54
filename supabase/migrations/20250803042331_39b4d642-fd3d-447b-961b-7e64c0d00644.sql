...
INSERT INTO public.turmas_ensino (nome_turma, curso_id, professor_responsavel, data_inicio, data_fim, dias_semana, horario_inicio, horario_fim, local_tipo, capacidade_maxima, status) 
SELECT 
...
-- Criar progresso nas trilhas DNA para o usuário admin (corrigindo os valores de status)
...
SELECT 
  ...
  END,
  CASE 
    WHEN ROW_NUMBER() OVER (ORDER BY t.id) <= 2 THEN '[1,2,3]'::jsonb
    ELSE '[1]'::jsonb
  END,
  CASE 
    WHEN ROW_NUMBER() OVER (ORDER BY t.id) = 1 THEN 'concluido'
